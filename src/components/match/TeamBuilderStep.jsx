...
import { generateBalancedTeams, calculateTeamStats, getDefaultPosition, getIdealDistribution, getEmptyFieldSpot } from '../../utils/teamBalancer'

function TeamBuilderStep({ match, canManage = false, deviceId, onRegisterAddPlayerHandler }) {
  ...
  useEffect(() => {
    const autoGenerate = async () => {
      if (!canManage) return // spectators never write team configs
    ...
    autoGenerate()
  }, [teamConfig, registrations, players, match.jugadoresPorEquipo, match._id, saveTeamConfig, canManage])
  ...
  const handleSwapTeam = async (playerId) => {
    ...
    // Drop the player onto an empty spot of the target half so they don't land
    // on top of an existing player (which would hide them).
    const others = teamConfig.asignaciones.filter(a => a.jugadorId !== playerId)
    const spot = getEmptyFieldSpot(newTeam, others)

    const updatedAssignments = teamConfig.asignaciones.map(a =>
      a.jugadorId === playerId
        ? { ...a, equipo: newTeam, coordenadaX: spot.x, coordenadaY: spot.y }
        : a
    )
  ...
  const handleViewPlayerInfo = (player, registration) => {
    const playerId = player._id || player.id
    const reg = registration || registrations.find(r => r.jugadorId === playerId)
    setSelectedPlayer(player)
    setSelectedRegistration(reg)
  ...
  }, [match._id, removeRegistration, deviceId])
  ...
  return (
    ...
    <SoccerField
      ...
      onPositionChange={canManage ? handlePositionChange : () => {}}
      onSwapTeam={canManage ? handleSwapTeam : () => {}}
      ...
      <div className="team-builder-layout-new">
        <div className="team-builder-teams-row">
          <TeamPanel
            ...
            onUnassign={canManage ? handleUnassignPlayer : undefined}
            onSwapTeam={canManage ? handleSwapTeam : undefined}
            onAddPlayer={canManage ? handleOpenJoinFromEmptySlot : undefined}
            onTeamNameChange={canManage ? handleTeamNameChange : undefined}
    ...
    <div className="team-builder-extras-row">
      <div className="inscription-step">
        ...
        <div className="player-list compact-list">
          {sortedSuplentes.map((registration, index) => {
            ...
            return (
              <PlayerCard
                ...
                onRemove={canManage ? handleRemoveExtraPlayer : undefined}
                onPromote={teamsHaveEmptySlots && canManage ? promoteSuplente : undefined}
          ...
          {canManage && sortedSuplentes.length < MAX_SUPLENTES && (
